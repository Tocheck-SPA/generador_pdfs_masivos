...
| **Base de imágenes de la fuente** | PENDIENTE (bloqueo real). Las rutas son relativas (`/imagenes_checklist/254/2098/chk_...jpg`); `app.tocheck.cl` responde 404, no está en la BD. Requiere el host/CDN/bucket real. Mientras tanto: `SOURCE_ASSET_LOCAL_DIR` (fallback local) permite pruebas/comparación. | `app/source/asset_resolver.py`; env `SOURCE_ASSET_BASE_URL` / `SOURCE_ASSET_LOCAL_DIR`. |
